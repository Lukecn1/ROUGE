...
# List of stopwords
stopwords_ = set(stopwords.words('danish'))
stopwords_.update([',', '.','-', ';','\n','\r', '\xa0', '.'])
...
def rouge_N(reference_summary, candidate_summary, n):

    set_ext = set(get_ngrams(candidate_summary, n))
    set_abs = set(get_ngrams(reference_summary, n))
...
def get_union_LCS(ref_sent, cand_sent):

    ref_tokens = get_ngrams(ref_sent, 1) 
    cand_tokens = get_ngrams(cand_sent, 1)
...
def union_lcs(ref_sentence, candidate_summary):

    lcs_u = list()

    for cand_sent in candidate_summary:
        lcs_u.append(get_union_LCS(ref_sentence, cand_sent))
...
def rouge_L(reference_summary, candidate_summary):

    final_lcs = 0 

    reference_sentences = nltk.sent_tokenize(reference_summary, language = 'danish')
    candidate_sentences = nltk.sent_tokenize(candidate_summary, language = 'danish')
    
    for ref_sent in reference_sentences:
        final_lcs += len(union_lcs(ref_sent, candidate_sentences))

    if len(reference_summary) < 1 or len(candidate_summary) < 1:
        return 0.0 
    
    recall = final_lcs / len(get_ngrams(reference_summary, 1)) 
    precision = final_lcs / len(get_ngrams(candidate_summary, 1)) 
    f_score = 0.0
